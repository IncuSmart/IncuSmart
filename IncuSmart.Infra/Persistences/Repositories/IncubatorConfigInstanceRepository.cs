...
namespace IncuSmart.Infra.Persistences.Repositories
{
    public class IncubatorConfigInstanceRepository : IIncubatorConfigInstanceRepository
    {
        ...
        public async Task Update(IncubatorConfigInstance instance)
        {
            var entity = await _dbContext.IncubatorConfigInstances.FirstOrDefaultAsync(x => x.Id == instance.Id);
            if (entity != null)
            {
                instance.Adapt(entity);
            }
        }
    }
}
